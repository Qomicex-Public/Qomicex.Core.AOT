...
internal class Resourcepack : LocalResourceBase
{
    ...
    public Resourcepack(HttpClient http, string gameDirectory, string version, bool versionSegmented, string apiKey)
    {
        _http = http;
    ...
    private static (string sha1, long cfHash) ComputeHashesForFolder(string folderPath)
    {
        ...
        string sha1 = Convert.ToHexString(SHA1.HashData(zipBytes)).ToLowerInvariant();
